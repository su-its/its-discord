  ...
  client.on(Events.InteractionCreate, async (interaction) => {
    ...
    try {
      if (command.isDMAllowed && !interaction.guild) {
        ...
        logger.info(`Command failed: ${interaction.commandName} | User: ${userId} | Guild: ${guildId}`);
      } else if ("authorization" in command) {
        const ok = await (command as AdminCommand).authorization.isSatisfiedBy(interaction);
        if (ok) {
          await command.execute(interaction);
          logger.info(`Command completed: ${interaction.commandName} | User: ${userId} | Guild: ${guildId}`);
        } else {
      ...
      const commandErrorMessage = {
        content: "コマンド実行時にエラーが発生しました．管理者にお問い合わせください．",
        ephemeral: true,
      };
