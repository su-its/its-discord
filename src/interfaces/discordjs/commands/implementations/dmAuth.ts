...
const dmAuthCommand: Command = {
  data: new SlashCommandBuilder()
    .setName("dmauth")
    .setDescription(
      "DM認証コマンド - 一度にすべての情報を入力して認証を完了させます",
    )
...
async function dmAuthCommandHandler(interaction: CommandInteraction) {
  ...
  try {
    ...
    const studentNumber = interaction.options.get("student_number", true)
      .value as string;
    const oldDepartment = interaction.options.get("department", true)
      .value as Department;
    ...
    if (result.isFailure()) {
      ...
      logger.warn(
        `Member registration failed for user ${interaction.user.id}: ${error.message}`,
      );
      return;
    }
    ...
    logger.info(
      `Member registration completed for user: ${interaction.user.id}, member ID: ${response.memberId}`,
    );
  } catch (error) {
    logger.error(
      `Error in DM auth command for user ${interaction.user.id}:`,
      error,
    );
    await interaction.editReply(
