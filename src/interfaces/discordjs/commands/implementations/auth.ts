...
async function authCommandHandler(interaction: CommandInteraction) {
  ...
  try {
    ...
    const authenticateMemberUseCase =
      diContainer.getAuthenticateMemberUseCase();
    ...
    if (result.isFailure()) {
      ...
      logger.warn(
        `Authentication failed for user ${interaction.user.id}: ${error.message}`,
      );
      return;
    }
    ...
    logger.info(
      `Authentication completed for user ${interaction.user.id}, member: ${response.memberName}`,
    );
  } catch (error) {
    logger.error(
      `Error in auth command for user ${interaction.user.id}:`,
      error,
    );
    await interaction.editReply(
